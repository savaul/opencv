...
frontalface = cv2.CascadeClassifier("Hand.Cascade.1.xml")		# frontal face pattern detection
...
while True:
	...
	if not faceFound:
		if lastface == 0 or lastface == 1:
			...
			fface = frontalface.detectMultiScale(aframe,1.3,4,(cv2.cv.CV_HAAR_DO_CANNY_PRUNING + cv2.cv.CV_HAAR_FIND_BIGGEST_OBJECT + cv2.cv.CV_HAAR_DO_ROUGH_SEARCH),(20,20))
